            ...
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (view.equals(mDrawer)) {
                    ...
                    if (initialState == State.Close) {
                        drawerLayoutParams.bottomMargin = -mDrawerLayoutContent.getMeasuredHeight()+hei;
                        isShowing = false;
                    } else {
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                SharedUtils.singleton().put("dispatchTouchEvent",false);
                isConsumed = processDown(event);
                break;
            case MotionEvent.ACTION_MOVE:
                if(!SharedUtils.singleton().get("dispatchTouchEvent",true)){
                    processMove(event);
                }
                break;
